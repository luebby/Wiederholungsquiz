...
Kann die generelle Zufriedenheit (`overall`) linear durch die Zufriedenheit mit den Fahrgeschäften (`rides`) sowie inwieweit Wochenende ist (`weekend`) modelliert werden?
...
Bei einem $R^2=`r rsquared(erg.lm)`$ wird $`r round(rsquared(erg.lm)*100)`\,\%$ der Variation der generellen Zufriedenheit (`overall`) modelliert.

Die Nullhypothese $\beta_1=0$ wird mit einem p-Wert $<2 \cdot 10^{-16}$ verworfen, $\beta_2=0$ wird mit einem p-Wert von $0.489$ nicht verworfen. Der Zusammenhang zwischen `rides` und `overvall` ist signifikant, der zwischen `weekend` und `overall` nicht - d.h. aber nicht, dass es keinen gibt!
